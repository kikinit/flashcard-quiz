...
describe('Question', () => {
  it('should create a question with text, options, and a correct answer', () => {
    const sut = new Question('What is 2 + 2?', ['3', '4', '5'], '4')

    expect(sut.getText()).toBe('What is 2 + 2?')
    expect(sut.getOptions()).toEqual(['3', '4', '5'])
  })

  it('should validate a correct answer', () => {
    const sut = new Question('What is 2 + 2?', ['3', '4', '5'], '4')

    expect(sut.checkAnswer('4')).toBe(true)
  })

  it('should invalidate an incorrect answer', () => {
    const sut = new Question('What is 2 + 2?', ['3', '4', '5'], '4')

    expect(sut.checkAnswer('3')).toBe(false)
  })

  it('should throw an InvalidAnswerError if the provided answer is not in the options', () => {
    const sut = new Question('What is 2 + 2?', ['3', '4', '5'], '4')
  
    expect(() => sut.checkAnswer('7')).toThrow(InvalidAnswerError)
  })
